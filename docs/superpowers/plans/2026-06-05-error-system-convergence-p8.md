...
- **D-P8-5 (lint/CI 전체 편입):** eslint files 글롭에 `apps/error-architecture/**/*.{ts,tsx}` 포함(ignores에서 제거), 벤더 허용 오버라이드 추가 — `apps/error-architecture/lib/composition-root.ts`(sonner presenter 조립) + `apps/error-architecture/app/providers.tsx`(Toaster 마운트). error-architecture-app에 lint 스크립트. CI `--filter` 제거 → 워크스페이스 전체 게이트.
