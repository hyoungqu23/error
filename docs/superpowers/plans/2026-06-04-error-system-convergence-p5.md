...
- **D-P5-4 (report-free Failure 경로 — 구현 중 교정):** safe-* Track1(expected)·VALIDATION은 report 없이 wire payload만 → `degrade<R>(errorSystem.finalizeUnknown(error, occurrence, { runtime: "server" }))` — finalizeUnknown은 결정+payload만 해소하고 텔레메트리를 실행하지 않는다. 원안(handleServerError + telemetry:false override)은 createHandleError가 override와 무관하게 executeErrorDecision을 무조건 호출해 report-free를 보장하지 못하고, 테스트의 `expect(handleServerError).not.toHaveBeenCalled()` 단정과 충돌해 기각(P5 리뷰 확정).
...
- `isDomainError`→`isAppError`, `isExpectedCode`→category==="business"(D-P5-3, `isKnownErrorCode` 가드). `actionFailure`→`degrade`(D-P5-4 교정안). VALIDATION 실패/Track1: `degrade<R>(errorSystem.finalizeUnknown(error, occurrence, {runtime:"server"}))` — report-free(텔레메트리 미실행). Track2: `handleServerError(error)`(present 제거) + throw. rethrowControlFlow 유지. FormState<R>=Result<R>|null 불변.
...
- **host 영향:** 옵션 1로 공개 prop 불변. apps(P8)는 의도된 red 유지 — P8 마이그레이션 대상 표면(P5 리뷰 열거): `serverReporter.health()` 손실(health route는 P6/P7 dead-man's-switch 재도입 후 재배선), `ErrorRegistryProvider`/`useErrorRegistry`/`toErrorResponse` 제거(composition-root/route 재배선).
