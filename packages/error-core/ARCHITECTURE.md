...
| `system.ts` | `createDecisionSystem` 팩토리 — finalize\*(D1 validateDetails 게이트)·**toClientErrorPayload(누출게이트)**·executeErrorDecision(텔레메트리 실행) | `createDecisionSystem`, `DecisionSystem`, `DecisionFailure`, `fail`, `ok` |
...
| `normalize.ts` | 경계 넘은 unknown → `AppError` 재수화 | `normalizeToAppError` |
...
| `network-boundary.ts` | 네트워크 경계(throwing 변환, 8개 transport 코드의 유일 생산자, zod 스키마 검증) | `networkBoundary` |
